...
def get_salary(post):
    try:
        ...
        if salary_text is None:
            ...
            if salary_max_t>=0:
                if salary_min_t>=0:
                    salary_min = int(salary_text[(salary_min_t + 3):salary_max_t])
                else:
                    salary_min = 0
                salary_max = int(salary_text[(salary_max_t + 3):])
            else:
                if salary_min_t>=0:
                    salary_min = int(salary_text[(salary_min_t + 3):])
                else:
                    salary_min = 0
                salary_max = 0
            ...
            if len(salary_text)>0 and salary_min_t<0 and salary_max_t<0:
                ...
                salary_min = int(salary_text[:sal])
                salary_max = int(salary_text[(sal+1):])
...
def get_page_strict_hh(soup, collection_hh):
    posts_list = []
    ...
    for post in posts_data:
        ...
        if collection_hh.count_documents({'post_url': post.find('a').attrs.get('href')})==0:
            collection_hh.insert_one(post_dict)
...
def get_page_strict_sj(soup, collection_sj):
    posts_list = []
    ...
    for post in posts_data:
        ...
        if collection_sj.count_documents({'post_url':f"{main_link_sj}{post_url}"})==0:
            collection_sj.insert_one(post_dict)

    return posts_list

def parser(job_name, pages, collection_hh, collection_sj):
    ...
    for i in range(1, num_pages):
        ...
        job_list.extend(get_page_strict_hh(soup_hh, collection_hh))
    ...
    for i in range(1, num_pages):
        ...
        job_list.extend(get_page_strict_sj(soup_sj, collection_sj))
...
def find_jobs(salary, collection_hh, collection_sj):

    for post in collection_hh.find({'post_salary_min':{'$gte': salary}}):
        pprint(post)

    for post in collection_hh.find({'post_salary_max':{'$gte': salary}}):
        pprint(post)

    for post in collection_sj.find({'post_salary_min':{'$gte': salary}}):
        pprint(post)

    for post in collection_sj.find({'post_salary_max': {'$gte': salary}}):
        pprint(post)
...
if __name__ == '__main__':
    ...
    db = client.jobs
    collection_hh = db.hh_vac
    collection_sj = db.sj_vac
    ...
    result_data = parser(job_name, pages, collection_hh, collection_sj)

    job_salary = input('Введите уровень зарплаты: ')
    find_jobs(job_salary, collection_hh, collection_sj)
